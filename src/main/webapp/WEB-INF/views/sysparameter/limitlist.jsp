...
<script >
    ...
    function initTable() {
        ...
        $("#cusTable").bootstrapTable({
            ...
            url: "<%=basePath%>/limit/getLimitList.do", //获取数据的Servlet地址
            striped: true,  //表格显示条纹
    ...
    $(document).ready(function () {
        ...
        //校验
        initValidate();
        $("#limitForm").submit(function(ev){ev.preventDefault();});//AJAX提交必修使用
        $('#submitBtn').click(function() {
            var bootstrapValidator = $("#limitForm").data('bootstrapValidator');//必须
        ...
        $(".close").click(function (){
            $('#limitForm').data('bootstrapValidator').resetForm(true);
        })
    });
    //校验
    function initValidate(){
        $('#limitForm').bootstrapValidator({
            message: '值不能为空',
            feedbackIcons: {
    ...
    //添加和编辑提交按钮
    function submit(){
        var limitLabel=$("#limitLabel").text();//添加编辑用户窗口
        if(limitLabel.indexOf("新增") !=-1){
            saveLimit();//添加提交
        } else{
            updateLimit();//修改提交
        }
    ...
    function editLimit(){
        ...
        if(arr.length>0) {
            ...
            if (uids.length > 1) {
                ...
                $.ajax({
                    type: "POST",
                    url: "<%=basePath%>/limit/getLimitByUid.do",
                    data: {
                        uid: uid
                    },
                    success: function (data) {
    ...
    function deleteUser(){
        ...
        $.ajax({
            type: "POST",
            url: "<%=basePath%>/limit/deleteLimitByUid.do",
            data: {
                uid:getCheckUid()
            },
            beforeSend : function() {
...
<div class=" style_border">
    <div id="toolbar" class="btn-group-sm">
        <button id="add" class="btn btn-info" onclick="newLimit()">
            <i class="glyphicon glyphicon-expand"></i> 增加
        </button>
        <button id="edit" class="btn btn-info" onclick="editLimit()">
...
<div class="modal fade" id="newLimit" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content" style="width:400px">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                <h4 class="modal-title" id="limitLabel"></h4>
            </div>
